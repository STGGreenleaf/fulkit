  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      glowGrad.addColorStop(
        0,
        `rgba(${config.color[0]}, ${config.color[1]}, ${config.color[2]}, ${alpha})`
      );
      glowGrad.addColorStop(
        0.5,
        `rgba(${config.color[0]}, ${config.color[1]}, ${config.color[2]}, ${alpha * 0.3})`
      );
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const pAlpha =
          0.1 + intensity * 0.2 + Math.sin(time * 2 + i * 0.5) * 0.05;
      ...
      for (let l = layers - 1; l >= 0; l--) {
        ...
        for (let i = 0; i <= points; i++) {
          ...
          const n2 =
            smoothNoise(nx * 2.5, ny * 2.5, time * 1.5 + l * 5) *
            intensity *
            0.3;
          const pulse =
            Math.sin(time * config.pulseSpeed * 60 + angle * 2) *
            0.015 *
            intensity;
        ...
        if (l === 0) {
          ...
          orbGrad.addColorStop(
            0,
            `rgba(${Math.min(c[0] + 30, 255)}, ${Math.min(c[1] + 30, 255)}, ${Math.min(c[2] + 25, 255)}, ${layerAlpha})`
          );
          orbGrad.addColorStop(
            0.5,
            `rgba(${c[0]}, ${c[1]}, ${c[2]}, ${layerAlpha * 0.8})`
          );
          orbGrad.addColorStop(
            1,
            `rgba(${c[0] - 40}, ${c[1] - 40}, ${c[2] - 45}, ${layerAlpha * 0.6})`
          );
      ...
      hlGrad.addColorStop(
        0,
        `rgba(255, 255, 250, ${0.08 + intensity * 0.06})`
      );
        ...
        <div style={{ display: "flex", gap: "var(--space-4)", alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              boxShadow:
                mode === "listening"
                  ? "0 0 20px rgba(196, 59, 46, 0.3)"
                  : "var(--shadow-none)",
            }}
            title={mode === "idle" ? "Start talking" : "Tap"}
          >
            {mode === "listening" ? (
              <div
                style={{
