...
const MOCK_PLANS = [
  { id: "free", name: "Free", price: "$0", period: "forever", msgs: 100, features: ["100 messages/mo", "Sonnet 2K context", "Basic vault"] },
...
function PlaygroundTab() {
  ...
  const [mockProfile, setMockProfile] = useState({ seat_type: "free", stripe_customer_id: null, stripe_subscription_id: null, messages_this_month: 23 });
  ...
  function advanceGateway(plan) {
    if (gwStep === 0) {
      ...
      // Simulate checkout complete → webhook
      setWebhookEvents(prev => [...prev, {
        type: "checkout.session.completed",
        time: new Date().toLocaleTimeString(),
        data: { plan: plan || selectedPlan, customer: "cus_mock_" + Math.random().toString(36).slice(2, 8), subscription: "sub_mock_" + Math.random().toString(36).slice(2, 8) },
      }]);
      setGwStep(2);
    } else if (gwStep === 2) {
      // Webhook processed → active
      const p = selectedPlan || "standard";
      setMockProfile(prev => ({
        ...prev,
        seat_type: p,
        stripe_customer_id: webhookEvents[webhookEvents.length - 1]?.data?.customer || "cus_mock",
        stripe_subscription_id: webhookEvents[webhookEvents.length - 1]?.data?.subscription || "sub_mock",
      }));
      setGwStep(3);
    } else if (gwStep === 3) {
      setGwStep(4);
      ...
      setWebhookEvents(prev => [...prev, {
        ...
        time: new Date().toLocaleTimeString(),
        data: { plan: selectedPlan, customer: mockProfile.stripe_customer_id },
      }]);
      setMockProfile(prev => ({ ...prev, seat_type: "free", stripe_subscription_id: null }));
      setGwStep(5);
    }
  }
  ...
  function resetGateway() {
    ...
    setWebhookEvents([]);
    setMockProfile({ seat_type: "free", stripe_customer_id: null, stripe_subscription_id: null, messages_this_month: 23 });
  }

  const stepStyle = (i) => ({
    fontSize: "var(--font-size-2xs)",
    fontFamily: "var(--font-mono)",
    fontWeight: i === gwStep ? "var(--font-weight-bold)" : "var(--font-weight-normal)",
    color: i < gwStep ? "var(--color-success)" : i === gwStep ? "var(--color-text)" : "var(--color-text-dim)",
  ...
  const btnStyle = (variant) => ({
    ...
    background: variant === "primary" ? "var(--color-text)" : "transparent",
    color: variant === "primary" ? "var(--color-bg)" : "var(--color-text-secondary)",
  ...
  const seatLimit = { free: 100, standard: 450, pro: 800 }[mockProfile.seat_type] || 100;
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-4)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
            <CreditCard size={18} strokeWidth={1.5} style={{ color: "var(--color-text-muted)" }} />
            <h3 style={{ fontSize: "var(--font-size-lg)", fontWeight: "var(--font-weight-semibold)", color: "var(--color-text)", margin: 0 }}>
              Payment Gateway
            </h3>
          </div>
          {gwStep > 0 && (
            <button onClick={resetGateway} style={{ ...btnStyle("ghost"), width: "auto", display: "flex", alignItems: "center", gap: "var(--space-1)", padding: "var(--space-1) var(--space-2)", fontSize: "var(--font-size-2xs)" }}>
          ...
          {GATEWAY_STEPS.map((s, i) => (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: "var(--space-1)" }}>
              ...
              {i < GATEWAY_STEPS.length - 1 && (
                <ArrowRight size={10} style={{ color: "var(--color-text-dim)", flexShrink: 0 }} />
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          {gwStep === 0 && (
            <div style={{ padding: "var(--space-4)" }}>
              <div style={{ fontSize: "var(--font-size-xs)", fontFamily: "var(--font-mono)", color: "var(--color-text-dim)", textTransform: "uppercase", letterSpacing: "var(--letter-spacing-widest)", marginBottom: "var(--space-3)" }}>
                Select a plan
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {MOCK_PLANS.filter(p => p.id !== "free").map((plan) => (
                  <button
                    key={plan.id}
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "baseline", gap: 2, flexShrink: 0 }}>
                      <span style={{ fontSize: "var(--font-size-lg)", fontWeight: "var(--font-weight-bold)", fontFamily: "var(--font-mono)", color: "var(--color-text)" }}>{plan.price}</span>
                      <span style={{ fontSize: "var(--font-size-2xs)", color: "var(--color-text-dim)" }}>{plan.period}</span>
                    </div>
                  </button>
                ))}
              </div>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
                  <Zap size={14} strokeWidth={1.8} style={{ color: "var(--color-warning)" }} />
                  <div>
                    <div style={{ fontSize: "var(--font-size-sm)", fontWeight: "var(--font-weight-semibold)", color: "var(--color-text)" }}>Fül up</div>
                    <div style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)" }}>100 messages on demand</div>
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
                  <span style={{ fontSize: "var(--font-size-lg)", fontWeight: "var(--font-weight-bold)", fontFamily: "var(--font-mono)", color: "var(--color-text)" }}>$2</span>
                  <span style={{ fontSize: "var(--font-size-2xs)", color: "var(--color-text-dim)" }}>once</span>
                </div>
              </button>
            </div>
          )}
          ...
          {gwStep === 1 && (
            <div style={{ padding: "var(--space-6)", textAlign: "center" }}>
              <div style={{
                ...
                <CreditCard size={22} strokeWidth={1.5} style={{ color: "var(--color-text-muted)" }} />
              </div>
              <div style={{ fontSize: "var(--font-size-xs)", fontFamily: "var(--font-mono)", color: "var(--color-text-dim)", textTransform: "uppercase", letterSpacing: "var(--letter-spacing-widest)", marginBottom: "var(--space-1)" }}>
                Stripe Checkout
              </div>
              <div style={{ fontSize: "var(--font-size-md)", fontWeight: "var(--font-weight-bold)", marginBottom: "var(--space-1)" }}>
                Fülkit {selectedPlan === "credits" ? "Credits" : selectedPlan === "pro" ? "Pro" : "Standard"}
              </div>
              <div style={{ fontSize: "var(--font-size-sm)", color: "var(--color-text-muted)", marginBottom: "var(--space-4)" }}>
                {selectedPlan === "credits" ? "$2.00 one-time" : selectedPlan === "pro" ? "$15.00/month" : "$7.00/month"}
              </div>
              <div style={{ padding: "var(--space-3) var(--space-4)", background: "var(--color-bg)", borderRadius: "var(--radius-sm)", marginBottom: "var(--space-4)" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)" }}>
                  <span>Email</span><span>collin@fulkit.app</span>
                </div>
              </div>
              ...
              {webhookEvents.map((evt, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "var(--space-1)" }}>
                    ...
                    <span style={{ color: "var(--color-text-dim)" }}>{evt.time}</span>
                  </div>
                  <div style={{ color: "var(--color-text-muted)", fontSize: "var(--font-size-2xs)", wordBreak: "break-all" }}>
                    {JSON.stringify(evt.data)}
                  </div>
                </div>
              ))}
              <div style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)", marginBottom: "var(--space-3)" }}>
                Webhook verified (signature valid). Processing: update profiles.seat_type → "{selectedPlan === "credits" ? "free" : selectedPlan}"
                {selectedPlan === "credits" && ", decrement messages_this_month by 100"}
              </div>
              <button onClick={() => advanceGateway()} style={btnStyle("primary")}>
                Process Webhook →
              </button>
            ...
            <div style={{ padding: "var(--space-4)" }}>
              ...
              <div style={{
                ...
                {Object.entries(mockProfile).map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "var(--space-1) 0", borderBottom: "1px solid var(--color-border-light)" }}>
                    <span style={{ color: "var(--color-text-muted)" }}>{k}</span>
                    <span style={{ color: k === "seat_type" ? "var(--color-success)" : "var(--color-text)", fontWeight: k === "seat_type" ? "var(--font-weight-bold)" : "var(--font-weight-normal)" }}>
              ...
              <div style={{ marginBottom: "var(--space-4)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "var(--space-1)" }}>
                  <span style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)" }}>Fül remaining</span>
                  <span style={{ fontSize: "var(--font-size-xs)", fontFamily: "var(--font-mono)", fontWeight: "var(--font-weight-bold)" }}>{remaining} / {seatLimit}</span>
                </div>
                <div style={{ height: 6, borderRadius: "var(--radius-full)", background: "var(--color-border-light)", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${Math.max(0, (remaining / seatLimit) * 100)}%`, borderRadius: "var(--radius-full)", background: "var(--color-accent)", transition: "width var(--duration-slow) var(--ease-default)" }} />
                </div>
              </div>
          ...
          {gwStep === 4 && (
            <div style={{ padding: "var(--space-6)", textAlign: "center" }}>
              <div style={{
                ...
                <Users size={22} strokeWidth={1.5} style={{ color: "var(--color-text-muted)" }} />
              </div>
              <div style={{ fontSize: "var(--font-size-xs)", fontFamily: "var(--font-mono)", color: "var(--color-text-dim)", textTransform: "uppercase", letterSpacing: "var(--letter-spacing-widest)", marginBottom: "var(--space-1)" }}>
              ...
              <div style={{ fontSize: "var(--font-size-sm)", color: "var(--color-text-muted)", marginBottom: "var(--space-4)" }}>
                {mockProfile.stripe_customer_id} · Fülkit {selectedPlan === "pro" ? "Pro" : "Standard"}
              </div>
              <div style={{ padding: "var(--space-3) var(--space-4)", background: "var(--color-bg)", borderRadius: "var(--radius-sm)", marginBottom: "var(--space-4)", textAlign: "left" }}>
                <div style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)", marginBottom: "var(--space-2)" }}>Actions available:</div>
                <div style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-secondary)", lineHeight: "var(--line-height-relaxed)" }}>
                  • Update payment method<br/>
                  • Change plan (Standard ↔ Pro)<br/>
                  • View billing history<br/>
                  • Cancel subscription
                </div>
              </div>
              <button onClick={() => advanceGateway()} style={{ ...btnStyle("ghost"), color: "var(--color-error)" }}>
                Cancel Subscription →
              </button>
            </div>
          )}
            ...
            <div style={{ padding: "var(--space-4)" }}>
              ...
              {webhookEvents.map((evt, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "var(--space-1)" }}>
                    <span style={{ color: evt.type.includes("deleted") ? "var(--color-error)" : "var(--color-success)", fontWeight: "var(--font-weight-bold)" }}>
                      ● {evt.type}
                    </span>
                    <span style={{ color: "var(--color-text-dim)" }}>{evt.time}</span>
                  </div>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "var(--space-1) 0" }}>
                  <span style={{ color: "var(--color-text-muted)" }}>seat_type</span>
                  <span style={{ color: "var(--color-text)", fontWeight: "var(--font-weight-bold)" }}>free</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "var(--space-1) 0" }}>
                  <span style={{ color: "var(--color-text-muted)" }}>stripe_subscription_id</span>
