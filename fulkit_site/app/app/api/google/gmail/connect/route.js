...
export async function GET(request) {
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL("/settings/sources?gm=error&reason=no_token", request.url));
    }
    ...
    if (!error && data?.user) {
      ...
      try {
        ...
        return NextResponse.redirect(new URL("/settings/sources?gm=error&reason=bad_token", request.url));
      }
    }
    ...
    console.error("[google/gmail/connect]", err.message);
    return NextResponse.redirect(new URL("/settings/sources?gm=error&reason=server", request.url));
  }
}
