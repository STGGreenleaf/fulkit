...
export async function GET(request) {
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL("/settings/sources?gd=error&reason=no_token", request.url));
    }
    ...
    if (!error && data?.user) {
      ...
      try {
        ...
        return NextResponse.redirect(new URL("/settings/sources?gd=error&reason=bad_token", request.url));
      }
    }
    ...
    console.error("[google/drive/connect]", err.message);
    return NextResponse.redirect(new URL("/settings/sources?gd=error&reason=server", request.url));
  }
}
