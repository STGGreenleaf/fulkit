...
export async function GET(request) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL("/settings/sources?monday=error&reason=no_token", request.url));
    ...
    if (!error && data?.user) { user = data.user; }
      ...
      try {
        ...
        user = adminData.user;
      } catch { return NextResponse.redirect(new URL("/settings/sources?monday=error&reason=bad_token", request.url)); }
    }
    ...
    return NextResponse.redirect(`https://auth.monday.com/oauth2/authorize?${params.toString()}`);
  } catch (err) {
    console.error("[monday/connect]", err.message);
    return NextResponse.redirect(new URL("/settings/sources?monday=error&reason=server", request.url));
  }
}
