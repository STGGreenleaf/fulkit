...
export async function GET(request) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL("/settings/sources?linear=error&reason=no_token", request.url));
    ...
    if (!error && data?.user) { user = data.user; }
      ...
      try {
        ...
        user = adminData.user;
      } catch { return NextResponse.redirect(new URL("/settings/sources?linear=error&reason=bad_token", request.url)); }
    }
    ...
    console.error("[linear/connect]", err.message);
