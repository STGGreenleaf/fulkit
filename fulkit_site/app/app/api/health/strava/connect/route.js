...
export async function GET(request) {
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL("/settings/sources?strava=error&reason=no_token", request.url));
    }
    ...
    if (!error && data?.user) {
      ...
      try {
        ...
        return NextResponse.redirect(new URL("/settings/sources?strava=error&reason=bad_token", request.url));
      }
    }
    ...
    console.error("[strava/connect]", err.message);
    return NextResponse.redirect(new URL("/settings/sources?strava=error&reason=server", request.url));
  }
}
