...
export async function GET(request) {
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL("/settings/sources?fb=error&reason=no_token", request.url));
    }
    ...
    if (!error && data?.user) {
      ...
      try {
        ...
        return NextResponse.redirect(new URL("/settings/sources?fb=error&reason=bad_token", request.url));
      }
    }
    ...
    return NextResponse.redirect(new URL("/settings/sources?fb=error&reason=server", request.url));
  }
}
