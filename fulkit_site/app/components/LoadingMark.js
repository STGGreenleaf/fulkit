...
const css = `
  ...
  .fk-rock-body {
    animation: fk-rock 1.8s ease-in-out infinite;
    transform-origin: 360px 360px;
  }
  .fk-wink-right {
    animation: fk-wink 3.6s ease-in-out infinite;
    transform-box: fill-box;
    transform-origin: center center;
  }
`;

export default function LoadingMark({ size = 50 }) {
  return (
    <>
      <style>{css}</style>
      <svg viewBox="0 0 720 720" width={size} height={size} style={{ overflow: "visible" }}>
        <g className="fk-rock-body">
          ...
          <rect className="fk-wink-right" fill={GRAY} x="394.87" y="125.69" width="73.01" height="73.01" />
        </g>
      </svg>
    </>
  );
