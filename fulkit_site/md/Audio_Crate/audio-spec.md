...
### Fullscreen Visualization — Deep Amoeba (v3)

Single renderer. No variants. The form is the Fulkit visual signature. Toggled via Maximize2 button in player controls. This is the ONLY thing on screen — full resources, full layer count, no competing UI.

**Prototype reference:** `fullscreen-viz-v3.jsx`
...
#### Overlay (Locked)
- Fulkit u + wordmark — top-left, 50% opacity
- X close — top-right, 35% opacity
...
#### Performance
- Only one renderer active at a time (amoeba replaces linear terrain)
- Canvas resizes to viewport on window resize
- Achromatic only. Motion is the design.
- 72 points (not 90+) — fewer points = bigger, more dramatic lobes
- 2 smoothing passes max — let the peaks be tall

#### What the Prototype Doesn't Have (Claude Code adds)

**Temporal Smoothing:** Prototype computes each frame independently. Real renderer needs per-point frame-to-frame interpolation (EMA or spring) so displacement eases between values. The `Math.random()` jitter should be replaced with noise-based jitter that's deterministic and smooth.

**Real Audio Data:** Prototype uses static feature values per track. Real system:
...
**Poster/Capture Mode:** Still TBD — capture current frame as PNG for export. Metadata overlay for poster version.

#### Key Learnings

1. **The amoeba is the brand.** 3D mesh experiments were interesting but don't belong to the same visual family as the horizontal waveform. The 2D tracer-based amoeba does.
...
3. **Color should be warm grey, not black.** Match the linear waveform's tone.
4. **Silent = perfect circle.** Light, thin, geometric. Music earns the organic form.
5. **No hairs/daggers.** Neighbor-smoothing catches isolated spikes. Guaranteed minimum control point offset on tendrils prevents straight lines.
6. **Boundary matters.** The form should never escape its visual zone.
...
- [ ] Pulse the Fulkit logo mark to the beat (very subtle)
...
**File:** `app/spotify/page.js` — fullscreen visualizer component
