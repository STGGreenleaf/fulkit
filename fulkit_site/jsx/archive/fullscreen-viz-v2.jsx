...
export default function FullscreenVizVariants() {
  ...
  useEffect(() => {
    ...
    const N = 90; // fewer points = bigger lobes
    const MAX_TRACE = 24;
    const MAX_HITS = 6;
    ...
    function draw() {
      ...
      const warpScale = v === "A" ? 0.35 : v === "B" ? 0.28 : 0.42;
      const noiseFreq1 = v === "A" ? 2.0 : v === "B" ? 1.6 : 2.5;
      const noiseFreq2 = v === "A" ? 5.0 : v === "B" ? 4.0 : 7.0;
      const noiseW1 = v === "A" ? 0.7 : v === "B" ? 0.8 : 0.6;
      const noiseW2 = v === "A" ? 0.12 : v === "B" ? 0.08 : 0.18;
      const dispScale = v === "A" ? 0.55 : v === "B" ? 0.45 : 0.65;
      ...
      for (let i = 0; i < N; i++) {
        ...
        const beatBoost = 1 + beat * 0.4;
        disp[i] = nv * s.amp * s.energy * beatBoost * exhale * baseR * dispScale;
        disp[i] *= (1 + (Math.random() - 0.5) * 0.05);
      }

      // Neighbor-smooth — kill isolated spike daggers
      // Each pass averages a point with its neighbors (wrapping)
      // Wide lobes survive (multi-point features), single-point spikes die
      ...
      if (beat > 0.65 && s.playing && s.frame % CAP_INTERVAL === 0) {
        ...
        for (let i = 0; i < N; i++) hd[i] = disp[i] * (v === "C" ? 1.8 : 1.5);
      ...
      for (const layer of layers) {
        ...
        // --- Dark outer edge (newest = darkest) ---
        // Invert: age 0 = full dark, older = lighter
        const edgeAlpha = alpha * (v === "B" ? 0.7 : 0.8);

        // Wide soft inner-bleed pass (connective tissue — NO outward halo)
        drawSmooth(ctx, pts);
        ctx.save();
        // Clip to inside the path to prevent outward halo
        ctx.clip();
        ctx.strokeStyle = `rgba(${col[0]},${col[1]},${col[2]},${edgeAlpha * 0.1})`;
        ctx.lineWidth = thisLw * (v === "B" ? 5 : 4);
        ctx.stroke();
        ctx.restore();

        // Sharp contour — this is the dark edge
        drawSmooth(ctx, pts);
        ctx.strokeStyle = `rgba(${col[0]},${col[1]},${col[2]},${edgeAlpha * (layer.age === 0 ? 0.85 : 0.55)})`;
        ctx.lineWidth = Math.max(0.3, thisLw * (layer.age === 0 ? 1.2 : 0.8));
        ctx.stroke();
